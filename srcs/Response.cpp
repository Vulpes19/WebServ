...
/*   Updated: 2023/07/27 16:47:02 by abaioumy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ErrorResponse::errorBadRequest( SOCKET socket, std::string path )
{
	...
	errorMsg << "\r\n";
	ssize_t bytesSent = send( socket, errorMsg.str().data(), errorMsg.str().size(), 0 );
...
enum ResponseStates	Response::handleCGI( Resources &resources, std::string path )
{
	...
	if ( !cgi.file.is_open() )
		return (RESET);
	ssize_t bytesSent = send( socket, "HTTP/1.1 200 OK\r\n", 17, 0 );
	if ( bytesSent == -1 || bytesSent == 0 )
	{
		cgi.file.close();
		err.errorInternal(socket, errorPages["500"]);
...
bool    Response::handleWriteResponse( Resources &resources )
{
	...
	bool isAllowed = false;	if ( allowedMethods.empty() )
	isAllowed = true;
